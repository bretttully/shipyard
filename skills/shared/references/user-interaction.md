...
Every skill that talks to you directly — `/sy:plan`, `/sy:spec`, `/sy:ship`, `/sy:spike`, `/sy:pr` — uses exactly one of three modes per turn and never blends them. A turn that narrates what happened, then somewhere in the middle needs a decision, then keeps narrating, is the failure mode this reference exists to prevent: the decision reads as one more sentence and gets missed. `/sy:ci` and `/sy:standards` never talk to you directly and are out of scope here.
...
A non-blocking nicety — suggesting `/rename`, pointing at a follow-up `/sy:plan` brief — is not an Action needed. It stays a single low-key aside, explicitly framed as optional, so it never competes with a real gate for your attention. If ignoring it changes nothing about whether the run proceeds, it does not earn a block.
